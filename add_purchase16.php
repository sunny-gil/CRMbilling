...
if (isset($_POST['submit'])) {
  ...
  if($_POST['redio']=='bag')
    ...
    $sqlpurchase = "INSERT INTO  `sealer_bill`(`bill_no`,`sealer_name`, `date`, `material_name`, `color`, `gsm`, `quantity`, `weight`, `rate`,` amount`, `gst`,`total_amount`, `paid_amount`, `balance_amount`,`note`,`taken_by`, `pay_state` )  VALUES ('$bill_no','$sealer_name', '$date', '$material_name', '$color', '$gsm', '$quantity', '$weight', '$rate', '$amount', '$gst', '$total_amount', '$paid_amount', '$balance_amount', '$note', '$taken_by', '$pay_state') ";
                    ...
                    <div class="row row-cols-1 row-cols-md-3 g-4">
                        ...
                        <div class="col">
                            <div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="date" value="<?php echo date("Y-m-d") ?>" class="form-control" id="orderDate" name="date" autocomplete="off" />
                                    </div>
                                </div>
                        ...
                        <div class="col">
                            <div>
                                ...
                                <div class="row mb-3">
                                    <label for="inputEmail3" class="col-sm-4 col-form-label">SELLER NAME</label>
                                    <div class="col-sm-8">
                                        <!--<select name="client_name" id="client" onchange="client()" onclick="client()" onkeyup="client()" style="width: 100%;" >-->
                                         <input type="text" class="form-control"  id="sealer_name" name="sealer_name" >
 ...
 <script type="text/javascript">
                    ...
                    $("#save12").on("click", function()
                    {
                        ...
                        $.ajax({
                          ...
                          data:{order_id:order_id,material_name:material_name, color:color, gsm:gsm, hsn:hsn, quantity1:quantity1, weight1:weight1, rate1:rate1, rate:rate, weight:weight, quantity:quantity, amount:amount, amount1:amount1, gst:gst, gst_amount:gst_amount},
                          dataType:"json",
                          success:function(response) {
                            if(response.done==1)
                            {
                                ...
                                $("tbody").append(`<tr class="tr`+response.idis+`"><td>`+id+`</td><td>`+material_name+`</td><td>`+color+`</td><td>`+gsm+`</td><td>`+hsn+`</td><td>`+quantity1+`</td><td>`+weight1+`</td><td>`+rate1+`</td><td>`+mult+`</td><td>`+gst+`</td><td>`+gst_amount+`</td><td><button class="btn btn-primary" onclick="removethis(`+response.idis+`)">delete</button></td></tr>`);
                                id = parseFloat(id)+1;
                                $("#id").val(id);
